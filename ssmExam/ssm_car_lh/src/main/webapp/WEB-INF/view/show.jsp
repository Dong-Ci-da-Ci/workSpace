            ...
            "columns":[
               ...
               {"data":"producedDate",
                ...
                {"data":"updateDates",
                    ...
                    "title":"修改时间"},
                {"data":"carPrice","title":"价格"},

				/*显示下拉列表只修改↓*/
                {"data":"brandName","title":"品牌"},
                {"data":"isup","title":"是否有票",
                    "render":function(data){
                        if(data == 1){
                            return data = "是";
                        }else{
        ...
        bootbox.confirm({
            ...
            callback:function(result){
                ...
                if(result){
                    // 获取参数
                    var param = {};
                    param.carName = $("#addForm #name").val();
                    param.carPrice = $("#addForm #price").val();
                    ...
                    param.configure = person.toString();
                    //单选框
                    param.isup = $("#addForm [name='status']:checked").val();
                    //图片
                    param.imgUrl = $("#addForm #img").val();


                    $.post({
                        url:"<%=request.getContextPath()%>/carController/addEntity",
                        data:param,
                        dataType:"json",
	...
	function initFileInput(id){
		$(id).fileinput({
			language:"zh",
	  			...
	  			//设置上传文件的地址
	  			uploadUrl:"<%=request.getContextPath()%>/carController/upload",
	  			// 上传图片需要传的值
