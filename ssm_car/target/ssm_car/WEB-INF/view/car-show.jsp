		...
		<form class="form-horizontal" role="form" method="post" id="updateForm">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
					...
					<input type="hidden" class="form-control" id="updateDate">
                </div>
            </div>
	...
	function update(carId){
		...
		$.post({
			...
			success:function(result){
                ...
                $("#updateForm #area").val(result.brandId);
				...
				bootbox.confirm({
					...
					// 获取值
                            ...
                            param["brandId"] = $("#updateForm #area").val();
                            var arr = $("#updateForm [name='configure']:checked");
                            var configure = [];
                            for (var i = 0; i < arr.length; i++) {
                                configure.push(arr[i].value);
...
<script type="text/javascript">
        ...
        bootbox.confirm({
            ...
            callback:function(result){
                ...
                if(result){
                    ...
                    param["brandId"] = $("#addForm #area").val();
	...
	function queryCloList(){
		userTable = $("#myTable").DataTable({
            ...
            "columns":[
                ...
                {"data":"brandName","title":"汽车品牌"},
                {"data":"isup","title":"是否上市",
                    "render":function(data){
                        if(data == 1){
                            return data = "是";
                        }else {
	...
	function deleteCar(data){
		bootbox.confirm({
			...
			callback:function(result){
				...
				if(result){
					$.post({
                        data:{"carId":data.toString()},
                        url:"<%=request.getContextPath()%>/carController/deleteCar",
						success:function(result){
							search();
							bootbox.alert("删除成功！");
						},
	...
	function deleteBath(){
		...
		$.post({
            ...
            url:"<%=request.getContextPath()%>/carController/deleteBatchCar",
			success:function(){
				search();
				bootbox.alert("删除成功！！！");
			},
	...
	function queryareaList(){
		$.post({
			url:"<%=request.getContextPath()%>/carController/queryPullDownList",
			dataType:"json",
			success:function(result){
                ...
                for (var i = 0; i < result.length; i++) {
                    $("#updateForm #area").append("<option value='"+result[i].brandId+"'>"+result[i].brandName+"</option>");
                }
    ...
    function importExcel(){
        $("#file").fileinput({
            language:"zh",
            uploadClass:"btn btn-danger", // 设置上传按钮的样式
            allowedFileExtensions:["xlsx"], // 设置上传的后缀
            maxFileCount:1, // 设置上传的最大个数
            //设置上传文件的地址
            uploadUrl:"<%=request.getContextPath()%>/carController/importExcel",
        })

        // 上传图片的回调函数
        $("#file").on("fileuploaded",function(event,data){ // data代表后台返回的值
            // 获取上传完图片后返回的值
            var map = data.response;
            // 判断图片上成功
            if(map.success){
        ...
        bootbox.confirm({
            ...
            callback:function(result){
                $("#importExcel").html(importExcelHTML);
            }
        })
    }
    ...
    function exportExcel(){
        ...
        // 给Form表单添加一个路径
        searchForm.action="<%=request.getContextPath()%>/carController/exportExcel";
        // 通过js代码提交form表单
        searchForm.submit();
    }
    ...
    // 导出exportExcelFreeMarker
    function exportWordFreeMarker(){
        // 获取条件查询的form表单
        var searchForm = document.getElementById("searchForm");
        // 给Form表单添加一个路径
        searchForm.action="<%=request.getContextPath()%>/carController/exportWordFreeMarker";
        // 通过js代码提交form表单
        searchForm.submit();
    }


    // 导出exportPdfFreeMarker
    function exportPdfFreeMarker(){
        // 获取条件查询的form表单
        var searchForm = document.getElementById("searchForm");
        // 给Form表单添加一个路径
        searchForm.action="<%=request.getContextPath()%>/carController/exportPdfFreeMarker";
        // 通过js代码提交form表单
        searchForm.submit();
    }
