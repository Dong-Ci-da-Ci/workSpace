...
<!-- 全局js -->
    ...
    function queryCloList(){
        userTable = $("#myTable").DataTable({
            ...
            "ajax":{
                "type":"post",
                "url":"<%=request.getContextPath()%>/movieController/queryList",
                "data":function(data){
                    //条件查询
					...
					//多选
                    var arr = $("[name='person']:checked");
                    var person = [];
                    arr.each(function(a,b){
                        person.push(b.value);
                    })

                    data.person = person.toString();
            ...
            // 渲染列表数据
            "columns":[
                {"data":"mid","title":'<input type="checkbox" class="btn btn-info btn-sm" onclick="qx(this)">',
                ...
                {"data":"num","title":"余票数量"},
                {"data":"price","title":"价格"},
                {"data":"areaName","title":"地区"},
                {"data":"status","title":"是否有票",
                    "render":function(data){
                        if(data == 1){
                            return data = "是";
                        }else{
        ...
        bootbox.confirm({
            ...
            callback:function(result){
                ...
                if(result){
                    ...
                    param.num = $("#addForm #num").val();
                    param.price = $("#addForm #price").val();
                    param.showTime = $("#addForm #showTime").val();
                    param.updateTime = $("#addForm #updateTime").val();
                    //下拉列表
                    ...
                    //图片
                    param.imgUrl = $("#addForm #img").val();


                    $.post({
                        url:"<%=request.getContextPath()%>/movieController/addEntity",
                        data:param,
                        dataType:"json",
			...
			callback:function(result){
				...
				if(result){
					$.post({
						...
						url:"<%=request.getContextPath()%>/movieController/deleteEntity",
						success:function(result){
							search();
							bootbox.alert("删除成功！");
						},
	...
	function queryareaList(){
		$.post({
			url:"<%=request.getContextPath()%>/movieController/queryPullDownList",
			dataType:"json",
			success:function(result){
